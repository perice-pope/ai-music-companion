...
| **Cloud sync (learner model)** | Practice-progress blob: reveal collection (concepts + artist names), per-key mastery stats, difficulty step, daily-warmup streak (day count + best score, #257). **No raw audio, no recordings.** Rides the taste-profile opt-in (progress data, one switch); push-only — local stays authoritative. | Supabase project | Yes | **OFF** (same opt-in as taste profile) | `apps/desktop/src/stores/syncStore.ts` (`syncLearnerModel`) |
