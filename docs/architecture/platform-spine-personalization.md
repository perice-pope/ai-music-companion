...
**Companion doc to:** [`architecture-v2.md`](./architecture-v2.md), [`platform-modules-addendum.md`](./platform-modules-addendum.md), and [`../design/story-phase4-musical-relevance.md`](../design/story-phase4-musical-relevance.md)
**Status:** Draft
**Date:** 2026-06-07
...
This is one of three shared spines that hold the platform together (the other two — **Commerce / Entitlements** and **Unified Content Format** — are specified separately; this doc references them by name and does not redefine them). The personalization spine is **one student musical profile, read by many consumers**: a single SQLite-backed record of stated preferences (genres, artists, goals, experience) that enriches every coaching output, the AI Listening Coach, and content tailoring. It is the connective tissue that makes a Bach etude feel relevant to a kid who loves Kendrick — without each module inventing its own profile.
...
- **Stated, editable, evolving.** Captured at onboarding (a quick "pick a few artists / genres / why you're here"), editable any time, and refined by `derived_signals` — lightweight feedback about which cross-genre connections the student engaged with. This is the same taste profile described in story-phase4 §3.2; this spine owns its *platform schema and lifecycle*, that story owns how the relevance engine *consumes* it.
...
This spine re-uses that grounding contract; it does not re-derive it.

---

## Privacy

Reconciled with the teacher-dashboard privacy posture (`../design/story-phase3-teacher-dashboard-privacy.md`) and its precedent `teacher-audit.md`:

- **What's stored:** preference data only — genres, artists, goals, coarse experience level, and engagement-derived signals. No birthdate (we keep the coarse `age_tier` enum, never a DOB, per the privacy decision doc §3.1). The profile is *preference* data, categorically less sensitive than a minor's audio or PII.
...
In short: story-phase4 builds the *relevance brain*; this spine guarantees the *profile it reads from is a single shared platform asset*, not a feature-local table.
